...
from aplikasi_inspeksi import OcrManager, VideoCaptureThread, read_ocr_crop
...
class PipelineTests(unittest.TestCase):
    ...
    def make_worker(self):
        ...
        worker.ocr_manager.process.is_alive.return_value = True
        return worker
    ...
    def result(self, worker, revision=0, current=True):
        ...
        worker._pending_ocr = (1, revision, 1, state, time.monotonic())
    ...
    def test_ocr_timeout_does_not_block(self):
        worker = self.make_worker()
        worker._pending_ocr = (1, 0, 1, {}, time.monotonic() - 31)
    ...
    def test_dead_ocr_does_not_block(self):
        worker = self.make_worker()
        worker.ocr_manager.poll.return_value = None
        worker.ocr_manager.process.is_alive.return_value = False
        worker._poll_ocr()
        self.assertTrue(worker._ocr_failed)
